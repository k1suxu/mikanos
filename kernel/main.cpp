...
const PixelColor kDesktopBGColor{45, 118, 237}; // デスクトップ背景色
const PixelColor kDesktopFGColor{255, 255, 255}; // カーソル色等
...
extern "C" void KernelMain(const FrameBufferConfig& frame_buffer_config) {
  ...
  const uint64_t xhc_mmio_base = xhc_bar.value & ~static_cast<uint64_t>(0xf); // 下位4ビットのフラグを削除
  Log(kDebug, "xHC mmio_base = %08lx\n", xhc_mmio_base);

  usb::xhci::Controller xhc{xhc_mmio_base};

  // Intel製のxHCの場合、Intel Panther PointチップセットのUSB ホストコントローラはデフォルトでEHCIで動作するため、xHCIモードへ変更する操作が必要
  if (0x8086 == pci::ReadVendorId(*xhc_dev)) {
